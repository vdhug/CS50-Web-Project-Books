...
def login():
    if request.method == "POST":
        ...
        if result is None:
        	return render_template("error.html", message="Username or password incorrect", notShow=True, url_return='/login', page_name='login')
        else:
...
def books():
    ...
    if request.method == "GET":
        books = db.execute("SELECT * FROM books").fetchmany(10)
